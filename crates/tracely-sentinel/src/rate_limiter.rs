...
mod tests {
    ...
    #[test]
    fn test_token_bucket_zero_capacity_rejected() {
        let err = TokenBucket::new(0, 5).unwrap_err();
        assert!(matches!(err, RateLimiterError::InvalidConfig(_)));
        assert!(err.to_string().contains("must be > 0"));
    }

    #[test]
    fn test_leaky_bucket_zero_capacity_rejected() {
        let err = LeakyBucket::new(0, 10).unwrap_err();
        assert!(matches!(err, RateLimiterError::InvalidConfig(_)));
        assert!(err.to_string().contains("must be > 0"));
    }
}
