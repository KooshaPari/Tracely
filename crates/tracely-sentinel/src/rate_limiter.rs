...
impl TokenBucket {
    ...
    ///
    /// # Errors
    /// Returns `RateLimiterError::InvalidConfig` if `capacity` is 0.
...
mod tests {
    ...
    #[test]
    fn test_token_bucket_zero_capacity_error() {
        let result = TokenBucket::new(0, 5);
        assert!(result.is_err());
        match result {
            Err(RateLimiterError::InvalidConfig(msg)) => {
    ...
    #[test]
    fn test_leaky_bucket_zero_capacity_error() {
    ...
    #[test]
    fn test_token_bucket_valid_capacity_succeeds() {
        let bucket = TokenBucket::new(1, 5).unwrap();
        assert_eq!(bucket.remaining(), 1);
    }

    #[test]
    fn test_leaky_bucket_valid_capacity_succeeds() {
        let bucket = LeakyBucket::new(1, 10).unwrap();
        // Initially pending is 0 and capacity is 1, so has_capacity is true
